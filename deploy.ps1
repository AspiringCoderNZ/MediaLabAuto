...
new-PSDrive -Name "DS001" -PSProvider "MDTProvider" -Root "C:\DeploymentShare" 
            -Description "MDT Deployment Share" -NetworkPath "\\SRV-Media\DeploymentShare" 
            -Verbose | add-MDTPersistentDrive -Verbose

#Import Applcations Adobe Reader into MDT share
Import-MDTApplication -path "DS001:\Applications" -enable "True" -Name "Adobe reader 9" 
        -ShortName "reader" -Version "9" -Publisher "Adobe" -Language "English" 
        -CommandLine "Reader.exe /sAll /rs /l" -WorkingDirectory ".\Applications\Adobe reader 9" 
        -ApplicationSourcePath "C:\adobe" -DestinationFolder "Adobe reader 9" -Verbose

#Import Applcations VLC  into MDT share
import-MDTApplication -path "DS001:\Applications" -enable "True" -Name "VideoLan vlc 1" 
        -ShortName "vlc" -Version "1" -Publisher "VideoLan" -Language "English" 
        -CommandLine "vlc.exe /s /v /qn" -WorkingDirectory ".\Applications\VideoLan vlc 1" 
        -ApplicationSourcePath "C:\vlc" -DestinationFolder "VideoLan vlc 1" -Verbose

#Import Applcations Google into MDT share
import-MDTApplication -path "DS001:\Applications" -enable "True" -Name "Google Chrome 1" 
                    -ShortName "Chrome" -Version "1" -Publisher "Google" -Language "English" 
                    -CommandLine "MsiExec.exe /i googlechrome.msi /qn" 
                    -WorkingDirectory ".\Applications\Google Chrome 1"
                    -ApplicationSourcePath "C:\google" -
                    DestinationFolder "Google Chrome 1" -Verbose

#Import Operating System into MDT share
import-mdtoperatingsystem -path "DS001:\Operating Systems" -SourcePath "C:\Windows 11" 
                        -DestinationFolder "Windows 11" -Verbose

#Define tasksquence, update CustomSetting and Bootstrap 
import-mdttasksequence -path "DS001:\Task Sequences" -Name "OS with app" 
                    -Template "Client.xml" -Comments "Deploying Windows 11" 
                    -ID "1" -Version "1.0" 
                    -OperatingSystemPath "DS001:\Operating Systems\Windows 11 Pro in Windows 11 install.wim" 
                    -FullName "Windows User" -OrgName "Media" -HomePage "about:blank" -Verbose
...
[Default]
DeployRoot=\\Server-Media\DeploymentShare
